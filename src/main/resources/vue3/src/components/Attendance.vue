...
import type {ResponseDataType, Attendance} from "../config/format.config";
...
* 表格数据存储
* */
let rebocres = ref<Attendance[]>([]);
/*
* 分页数据存储备份
* */
let seach = ref<Page<Attendance>>({})
/*
* 分页数据
* */
let seach1 = ref<{
...
* 分页数据传输EMIT
* */
function pagination(page: Page<Attendance>) {
  seach1.value = {
...
function page(page: Page<Attendance>) {
  let promise = requestType.get(RequestOperate.PAGE)!(DataTypes.Attendance,page).then(response => {
...
function format(value:Attendance[]){
  value.forEach(ele=>{
    for (let eles in ele ){
      if (redio[eles]){
...
<template>
  <el-scrollbar>
    <Operate @selector="operate" :delete-ele="selectDeleteEle" :placeholders="_Attendance.name" :data-type="DataTypes.Attendance" @requestPage="page" :seach="seach1"/>
    <Data :data="rebocres"  :seach="seach1" @requestPage="page" :data-type="DataTypes.Attendance" @selectHander="selectHander">
      <template #data>
        <el-table-column fixed prop="type" label="请假类型"/>
        <el-table-column prop="name" label="备注"/>
        <el-table-column prop="astatus" label="是否迟到"/>
        <el-table-column prop="abortTime" label="请假截止时间"/>
        <el-table-column prop="userPojo.name" label="用户"/>
      </template>
    </Data>
