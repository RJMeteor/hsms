...
import type {ResponseDataType, Role} from "../config/format.config";
import {requestType} from "../config/edit.request";
import {Interface} from "../config/format.config";
...
* 表格数据存储
* */
let rebocres = ref<Role[]>([]);
/*
* 分页数据存储备份
* */
let seach = ref<Page<Role>>({})
/*
* 分页数据
* */
let seach1 = ref<{
...
* 分页数据传输EMIT
* */
function pagination(page: Page<Role>) {
  seach1.value = {
...
* page请求
* */
function page(page: Page<Role>) {
  let promise = requestType.get(RequestOperate.PAGE)!(DataTypes.Role,page).then(response => {
...
<template>
  <el-scrollbar>
    <Operate @selector="operate" :delete-ele="selectDeleteEle" :placeholders="_Role.name" :data-type="DataTypes.Role" @requestPage="page" :seach="seach1"/>
    <Data :data="rebocres"  :seach="seach1" @requestPage="page" :data-type="DataTypes.Role" @selectHander="selectHander">
      <template #data>
        <el-table-column fixed prop="name" label="角色名字"/>
