...
import type {ResponseDataType, Department} from "../config/format.config";
import {requestType} from "../config/edit.request";
import {Bad} from "../config/format.config";
...
* 表格数据存储
* */
let rebocres = ref<Department[]>([]);
/*
* 分页数据存储备份
* */
let seach = ref<Page<Department>>({})
/*
* 分页数据
* */
let seach1 = ref<{
...
* 分页数据传输EMIT
* */
function pagination(page: Page<Department>) {
  seach1.value = {
...
* page请求
* */
function page(page: Page<Department>) {
  let promise = requestType.get(RequestOperate.PAGE)!(DataTypes.Department,page).then(response => {
...
function format(value:Department[]){
  value.forEach(ele=>{
    for (let eles in ele ){
      if (redio[eles]){
...
<template>
  <el-scrollbar>
    <Operate @selector="operate" :delete-ele="selectDeleteEle" :placeholders="_Department.name" :data-type="DataTypes.Department" @requestPage="page" :seach="seach1"/>
    <Data :data="rebocres"  :seach="seach1" @requestPage="page" :data-type="DataTypes.Department" @selectHander="selectHander">
      <template #data>
        <el-table-column fixed prop="name" label="名称"/>
        <el-table-column prop="phone" label="电话"/>
        <el-table-column prop="email" label="邮箱"/>
        <el-table-column prop="address" label="地址"/>
        <el-table-column prop="commuting" label="上下班时间"/>
      </template>
    </Data>
