...
import type {ResponseDataType, Classification} from "../config/format.config";
...
* 表格数据存储
* */
let rebocres = ref<Classification[]>([]);
/*
* 分页数据存储备份
* */
let seach = ref<Page<Classification>>({})
/*
* 分页数据
* */
let seach1 = ref<{
...
* 分页数据传输EMIT
* */
function pagination(page: Page<Classification>) {
  seach1.value = {
...
* page请求
* */
function page(page: Page<Classification>) {
  let promise = requestType.get(RequestOperate.PAGE)!(DataTypes.Classification,page).then(response => {
...
function format(value:Classification[]){
  value.forEach(ele=>{
    for (let eles in ele ){
      if (redio[eles]){
...
<template>
  <el-scrollbar>
    <Operate @selector="operate" :delete-ele="selectDeleteEle" :placeholders="_Classification.name" :data-type="DataTypes.Classification" @requestPage="page" :seach="seach1"/>
    <Data :data="rebocres"  :seach="seach1" @requestPage="page" :data-type="DataTypes.Classification" @selectHander="selectHander">
      <template #data>
