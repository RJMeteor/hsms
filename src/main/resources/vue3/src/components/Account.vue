<script lang="ts" setup>
    ...
    import type {ResponseDataType, Account, requetParamsType} from "../config/format.config";
    ...
    * 表格数据存储
    * */
    let rebocres = ref<Account[]>([]);
    /*
    * 分页数据存储备份
    * */
    let seach = ref<Page<Account>>({})
    /*
   * 分页数据
   * */
    let seach1 = ref<{
    ...
    * page请求
    * */
    function page(page: Page<Account>) {
        let promise = requestType.get(RequestOperate.PAGE)!(DataTypes.Account,page).then(response => {
...
<template>
    <el-scrollbar>
        <Operate @selector="operate" :delete-ele="selectDeleteEle" :placeholders="_Account.name" :data-type="DataTypes.Account" @requestPage="page" :seach="seach1"/>
        <Data :data="rebocres"  :seach="seach1" @requestPage="page" :data-type="DataTypes.Account" @selectHander="selectHander">
            <template #data>
                <el-table-column fixed prop="name" label="账户名"/>
                <el-table-column prop="person" label="账户人"/>
